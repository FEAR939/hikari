<script lang="ts">
    ...
    async function getFiles() {
        ...
        files = files.filter(
            (file) =>
                String(file).endsWith(".mp4") || String(file).endsWith(".mkv"),
        );

        episodes = files || [];

        fullPathEpisodes = files.map((file) => `${$fileSelectedPath}/${file}`);
    }
...
<FileUtilModal bind:show={fileUtilModalShow} bind:files={fullPathEpisodes}
></FileUtilModal>
