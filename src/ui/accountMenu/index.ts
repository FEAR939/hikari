...
export function accountMenu() {
  ...
  menu.className =
    "absolute z-5 top-10 right-0 h-fit w-64 p-2 opacity-0 bg-[#080808] rounded-2xl space-y-1 scale-75 transition-all ease-in-out duration-150";
  ...
  avatar.classList.add("m-2", "mb-4");

  menu.appendChild(avatar);

  const settingsLink = document.createElement("div");
  settingsLink.className =
    "w-full px-2 py-1 rounded-md text-sm text-[#e0e0e0] flex items-center space-x-2 cursor-pointer";
